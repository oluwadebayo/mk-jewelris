...
export default async function handler(req, res) {
  ...
  if (!user) {
    // don't reveal whether email exists — return generic success (but for dev you can return 404)
  ...
  const link = `${frontendBase}/reset-password?token=${resetToken}`;

  // email template (keeps your dark theme style / brand)
  const html = `
  ...
  // attempt to send email (if mail config missing, log and still respond success)
  try {
